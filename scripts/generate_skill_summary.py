...
SKILL_REGISTRY: dict[str, dict] = {
    ...
    "bleed": {
        ...
        "status": "已实现",
        "trigger": "攻击命中 / 回合开始结算",
        "logic": "攻击时 target.bleed += getSkillLevel(bleed)（默认1）；敌方回合开始 resolveBleed 扣血并层数减半。可打总部。",
        "params": "desc「流血N」",
    ...
    "poison": {
        ...
        "status": "已实现",
        "trigger": "攻击命中",
        "logic": "target.poison += getSkillLevel(poison)（默认2）。中毒单位不参与强运判定。",
        "params": "desc「中毒N」",
        "files": "gameEngine.ts applyDamage, hasLuckyOnBoard",
    },
    "poisonBurst": {
        "name": "毒爆",
        "status": "已实现",
    ...
    "stealth": {
        ...
        "status": "部分实现",
        "trigger": "部署",
        "logic": "单位 isStealthed=true；法术选目标未检查隐踪，仅检查 immune。",
        "params": "无",
        "files": "gameEngine.ts cardToUnit",
    },
    "ambush": {
        "name": "伏击",
        "status": "已实现",
    ...
    "manaPierce": {
        ...
        "logic": "applyDamage / castSpell 中与 magicPierce 等价处理。",
        "params": "无",
        "files": "gameEngine.ts applyDamage, castSpell",
    },
    "magicPierce": {
    ...
    "magicDmg": {
        ...
        "status": "已实现",
        "trigger": "施放法术",
        "logic": "默认 2 伤；法力风暴 6；毒针 1（受伤+1）；天火降临固定 12 点打总部；混乱风暴对全场所有单位 1 伤（单独分支）。",
        "params": "部分按卡名硬编码",
